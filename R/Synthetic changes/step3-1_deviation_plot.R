...
## Load data ---------------------------------------------------------------------------------------
fit_res = readRDS("./data/fit_brms_result_synthetic.Rds")
...
#       
#       figure = scatter_deviation(t_score[, var_ind], df_proportion, name)
#     }
#   )
...
# save files
saveRDS(t_score_list, "./data/t_score_add.Rds")

i = 0

i = i + 1
print(i)
target = figures[(i*6-5):(i*6)]
file_name = names(target)[1] %>% 
  strsplit(., "-")
file_name = file_name[[1]][1:2] %>% 
  paste0(., collapse = "_") %>% 
  paste0("./figures/synthetic/dev_", ., ".pdf")

pdf(file_name, width=20, height=30)
